      ...
      <v-data-table :search="search" :headers="headers" :items="mDataArray">
        ...
        <template v-slot:[`item.options`]="{ item }">
          ...
          <v-chip color="error" @click="deleteItem(item)" small>
            ...
            ลบ</v-chip
          >
          <v-chip color="info" @click="opdItem(item)" small>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "เบอร์โทรศัพท์", value: "customer_tel" },
        { text: "เพิ่มเมื่อ", value: "createdAt" },
        // { text: "สถานะ ", value: "customer_status_id" },
  ...
  methods: {
    editItem(item) {
      this.$router.push(`/customer-edit/${item.customer_id_pri}`);
    },
    opdItem(item){
      this.$router.push(`/customer-opd/${item.customer_id_pri}`)
    },
